...
parser.add_argument(
    "--metadata",
    default=None,
    help="path to metadata file with column named 'sampleID'",
)
parser.add_argument(
    "--colorby",
    default=None,
    help="<optional> column name for a metadata field to color points. \
...
if colorby == None:
    ...
    for key, group in ld.groupby(colorby):
        ...
        p.scatter(
            x="mean1", y="mean2", size=7, color=cmap[key], legend_label=key, source=data
        )
