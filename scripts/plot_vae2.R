...
################# ag1000g ################
##########################################
a <- fread("out/ag1000g_phase1_500kSNPs_latent_coords.txt",header=T)
names(a) <- c("LD1","LD2","sampleID")
pc <- fread("out/ag1000g_phase1_500kSNPs_pca.txt")[,c(1,2,21)]
names(pc) <- c("LD1","LD2","sampleID")
...
#final prediction
pdf("out/ag1000g_phase1_100kSNPs_vae.pdf",width=6,height=2.5,useDingbats = F)
...
a <- fread("out/hgdp_chr1_1e6snps_latent_coords.txt",header=T)
names(a) <- c("LD1","LD2","sampleID")
pc <- fread("out/hgdp_chr1_1e6snps_pca.txt")[,c(1,2,21)]
names(pc) <- c("LD1","LD2","sampleID")
...
#final prediction
pdf("out/hgdp_chr1_1e6snps_vae_v_pca.pdf",useDingbats = F,width=6,height=2.5)
...
pdf("out/hgdp_chr1_1e6snps_vae_map.pdf",useDingbats = F,width=5.5,height=2.75)
load("~/locator/locator_hgdp/cntrymap.Rdata")
map <- crop(map,c(-170,170,-50,71))
...
library(gganimate)
a2 <- fread("out/hgdp_chr1_1e6snps_training_preds.txt")
names(a2) <- c("LD1","LD2","sampleID","epoch")
...
ggplot(c2,aes(x=LD2,y=-LD1,fill=region))+
  ...
  transition_manual(epoch)+
  labs(title='Epoch: {frame}')
anim_save("out/hgdp_chr1_1e6snps_training_preds.gif")
